...
<a href="/admin/cache?action=detail&cacheName=<%=cacheName%>">Region Detail</a>
| <a href="/admin/cache">All Regions</a>
...
%>
        <tr>
            ...
            <td>
             <a href="/admin/cache?action=item&cacheName=<%=cacheName%>&key=<%=element.getKey()%>"> View </a>
            | <a href="/admin/cache?action=remove&cacheName=<%=cacheName%>&key=<%=element.getKey()%>"> Remove </a>
            </td>
        </tr>
<%
    }
...
below. You can also <a href="javascript:decision('Clicking OK will clear all the data from all regions!','/admin/cache?action=clearAllRegions')">Clear all regions</a>
which empties the entire cache.
</p>
<p>
	<form action="/admin/cache">
...
%>
        <tr>
			<td>
                <a href="javascript:decision('Clicking OK will remove all the data from the region [<%=record.getCache().getCacheName()%>]!','/admin/cache?action=clearRegion&cacheName=<%=record.getCache().getCacheName()%>')" class="warning"> Clear </a>
            </td>
            <td> <%=record.getCache().getCacheName()%> </td>
            <td> <%=record.getCache().getSize()%> </td>
            <td> <%=record.getByteCount()%> </td>
            <td> <%=record.getByteCount() / ONE_MEGABYTE%> </td>/
            ...
            <td>
                <a href="/admin/cache?action=regionSummary&cacheName=<%=record.getCache().getCacheName()%>"> Summary </a>
                | <a href="/admin/cache?action=detail&cacheName=<%=record.getCache().getCacheName()%>"> Detail </a>
                | <a href="javascript:decision('Clicking OK will remove all the data from the region [<%=record.getCache().getCacheName()%>]!','/admin/cache?action=clearRegion&cacheName=<%=record.getCache().getCacheName()%>')"> Clear </a>
            </td>
        </tr>
<%
    }
